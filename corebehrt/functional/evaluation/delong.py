...
def fastDeLong(predictions_sorted_transposed, label_1_count):
    ...
    if k == 1:
        ...
        sx = np.var(v01, ddof=0)
        sy = np.var(v10, ddof=0)
        delongcov = np.array([[sx / m + sy / n]])
    else:
        sx = np.cov(v01, rowvar=False, ddof=0)
        sy = np.cov(v10, rowvar=False, ddof=0)
        if sx.ndim == 0:
            sx = np.array([[sx]])
        if sy.ndim == 0:
            sy = np.array([[sy]])
...
def delong_roc_std(ground_truth, predictions):
    """
    Computes ROC AUC standard deviation for a single set of predictions
    Args:
       ground_truth: np.array of 0 and 1
       predictions: np.array of floats of the probability of being class 1
    """
    auc, delong_cov = delong_roc_variance(ground_truth, predictions)
    # Take the square root of the diagonal elements of the covariance matrix
...
def delong_roc_auc(ground_truth, predictions):
    """
    Computes ROC AUC for a single set of predictions
    Args:
       ground_truth: np.array of 0 and 1
       predictions: np.array of floats of the probability of being class 1
    """
...
def delong_roc_auc_std(ground_truth, predictions):
    ...
    Args:
       ...
       predictions: np.array of floats of the probability of being class 1
    """
    _, std_dev = delong_roc_std(ground_truth, predictions)
    return std_dev
